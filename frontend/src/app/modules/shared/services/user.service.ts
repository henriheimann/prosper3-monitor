...
export class UserService {
  ...
  getAll(): BehaviorSubject<UserModel[] | null> {
    this.httpClient.get<UserModel[]>(`${environment.backendUrl}/users`)
  ...
  deleteByUserName(username: string): Observable<void> {
    return this.httpClient.delete<void>(`${environment.backendUrl}/users/${username}`)
      .pipe(tap(() => this.getAll()));
  }

  createUser(userModel: UserModel): Observable<UserModel> {
    return this.httpClient.post<UserModel>(`${environment.backendUrl}/users`, userModel)
      .pipe(tap(() => this.getAll()));
  }

  editUser(username: string, userModel: Partial<UserModel>) {
    return this.httpClient.put<UserModel>(`${environment.backendUrl}/users/${username}`, userModel)
      .pipe(tap(() => this.getAll()));
  }
}
